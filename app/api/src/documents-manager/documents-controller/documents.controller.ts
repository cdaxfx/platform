...
export class DocumentsController {
    ...
    @Post('/client/:uuid')
    @UseInterceptors(FileInterceptor('file'))
    async addDocumentToClient(@Param('uuid') uuid: string, @Body() data: AddClientDocumentDto, @Request() req) {
        const client = await this.clientsRepository.findOne({ uuid });
        if (!client)
            throw new BadRequestException({ message: 'Client not found.' });

        const document = await this.documentsRepository.findOne({uuid: data.documentUuid});
        if (!document)
            throw new BadRequestException({ message: 'Document not found.' });

        const clientDocument = await this.documentsService.addDocumentToClient(client, data.type, document);
    ...
    async deleteDocument(@Body() data: RemoveClientDocumentDto) {
        ...
        const clientDocument = await this.documentsService.findClientDocumentByDocument(doc);
        if (!clientDocument)
